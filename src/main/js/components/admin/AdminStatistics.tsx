...
export default class AdminStatistics extends React.Component<{}, {}> {
    render() {
		...
		subscriptionHeaderCell.push(<td className="admin-statistics-table-header-cell">Razdoblje</td>);
		subscriptionHeaderCell.push(<td className="admin-statistics-table-header-cell">Tip pretplate</td>);
		subscriptionHeaderCell.push(<td className="admin-statistics-table-header-cell">Sumarno naplaćeno (kn)</td>);
		subscriptionHeaderRows.push(<tr id="header_row">{subscriptionHeaderCell}</tr>);
		let subscriptionRows = [];
		for (var i = 0; i < adminStore.adminStatisticsDTO.subscriptionStatistics.length; i++){
			let rowID = `row${i}`;
			let subscriptionCell = [];
			subscriptionCell.push(<td id="cell1" className="admin-statistics-table-body-cell">{adminStore.adminStatisticsDTO.subscriptionStatistics[i].yyyymm}</td>);
			subscriptionCell.push(<td id="cell2" className="admin-statistics-table-body-cell">{adminStore.adminStatisticsDTO.subscriptionStatistics[i].subscriptionType}</td>);
			subscriptionCell.push(<td id="cell3" className="admin-statistics-table-body-cell">{adminStore.adminStatisticsDTO.subscriptionStatistics[i].sumPerType}</td>);
			subscriptionRows.push(<tr key={i} id={rowID} style={{borderBlockStyle: 'solid'}}>{subscriptionCell}</tr>);
	    }

		let diskSpaceHeaderRows = [];
		let diskSpaceHeaderCell = [];
		diskSpaceHeaderCell.push(<td id="header_cell1" className="admin-statistics-table-header-cell">Particija</td>);
		diskSpaceHeaderCell.push(<td id="header_cell2" className="admin-statistics-table-header-cell">Slobodnog prostora</td>);
		diskSpaceHeaderCell.push(<td id="header_cell3" className="admin-statistics-table-header-cell">Zauzeto</td>);
		diskSpaceHeaderCell.push(<td id="header_cell3" className="admin-statistics-table-header-cell">Ukupno</td>);
		diskSpaceHeaderRows.push(<tr id="header_row">{diskSpaceHeaderCell}</tr>);
		let diskSpaceRows = [];
